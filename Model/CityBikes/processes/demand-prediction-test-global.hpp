...
#include "../../CityBikes.Model/prediction/demand-processing.hpp"
...
namespace CityBikes::Processes
{
	...
	void testDemandPredictionGlobal(size_t timeFrames)
	{
		...
		for (auto& example : testExamples)
		{
			...
			for (size_t selectedInstances = 1; selectedInstances < otherExamples.size(); selectedInstances++)
			{
				...
				Data::Demand::CumulativeDemandPrediction<Nodes> selectedDaysActionsDemand = Model::Prediction::DemandAnalysis<Nodes>::accumulateDemand(
					Model::Prediction::DemandProcessing<Nodes>::computeDemand(
						Model::Prediction::DemandProcessing<Nodes>::computeGlobalDemand(
