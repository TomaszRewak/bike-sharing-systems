...
namespace CityBikes::Processes
{
	...
	void testRedistributionUnits(size_t timeFrames)
	{
		...
		for (auto& example : testExamples)
		{
			...
			for (int teamSize = 0; teamSize < 10; teamSize++)
			{
				std::cout << teamSize << '\t';

				Data::Relocation::RelocationTeam relocationTeam;

				for (size_t teamUnit = 0; teamUnit < teamSize; teamUnit++)
				{
					relocationTeam.push_back(Data::Relocation::RelocationUnit(70 * teamUnit / teamSize, 8, 10));
				}
				...
				for (const auto& demandPrediction : demandPredictions)
				{
					...
					std::cout 
						<< DataProcessing::Validation::AvailabilityValidation<Nodes>::getTimeOfUnavailability(relocationPrediction) << "\t"
						<< flowRelocationSimulation.getUnhandledCustomers() << "\t";
				}
				...
				std::cout 
					<< DataProcessing::Validation::AvailabilityValidation<Nodes>::getTimeOfUnavailability(sameDayRelocationPrediction) << "\t"
					<< flowRelocationSimulation.getUnhandledCustomers() << "\t";
				...
				std::cout 
					...
					<< distributionSimulationCopy.getUnhandledCustomers() << std::endl;
			}
		}
