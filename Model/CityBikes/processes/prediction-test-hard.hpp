				...
				for (size_t j = 0; j < 8; j++)
				{
					...
					for (size_t timeFrame = 0; timeFrame < timeFrames - window; timeFrame++)
					{
						for (size_t attempt = 0; attempt < attempts; attempt++)
						{
							...
							Data::FillLevel::FillLevelPrediction<Nodes> ownPrediction = Model::HardFlowPredictionSimulation<Nodes>::predict(
					...
					std::cout << offset / (timeFrames - window) / attempts << "\t";
				}
