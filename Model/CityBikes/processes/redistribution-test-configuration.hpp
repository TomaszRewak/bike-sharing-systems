...
namespace CityBikes::Processes
{
	...
	void testRedistributionConfiguration(size_t timeFrames)
	{
		...
		for (auto& example : testExamples)
		{
			...
			for (int minValue = -2; minValue < 10; minValue++)
				for (int maxValue = minValue + 1; maxValue < 30; maxValue++)
				{
					...
					for (const auto& demandPrediction : demandPredictions)
					{
						...
						std::cout << DataProcessing::Validation::AvailabilityValidation<Nodes>::getTimeOfUnavailability(relocationPrediction) << "\t";
					}
					...
					std::cout << DataProcessing::Validation::AvailabilityValidation<Nodes>::getTimeOfUnavailability(sameDayRelocationPrediction) << "\t";
					...
					std::cout << DataProcessing::Validation::AvailabilityValidation<Nodes>::getTimeOfUnavailability(realPrediction) << std::endl;
				}
		}
